...
cat(fmt_str, "> ns <- getNamespace(\"ParaAnita\")", "> ns\n", sep = "\n")
ns <- getNamespace("ParaAnita") |> print()
...
cat(fmt_str, "\n# From ns-internals {base} examples\n", sep = "")
...
utils::str(allinfoNS("ParaAnita")) |> print()

cat(fmt_str, "\n\n", sep = "")
citation("ParaAnita") |> print()

help(package = (nsn))

rm(allinfoNS, fmt_str, lsNamespaceInfo, ns, ns_exp, ns_lzd, ns_names, ns_pkg_env, ns_S3methods, nsn, pkg_names)
